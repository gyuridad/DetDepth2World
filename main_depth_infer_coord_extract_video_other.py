...
def imwrite_unicode(path, img, params=None):
    ...
    if not ok:
        return False
    buf.tofile(str(path))  # Windows 한글 경로 대응
    return True

def extract_frames(
    video_path,
    out_dir,
    every_n=1,             # n프레임마다 저장 (기본)
    target_fps=None,       # 또는 초당 몇 장 추출할지 지정 (둘 중 하나만 사용)
    start_sec=None,
    end_sec=None,
    resize=None,           # (W, H)로 리사이즈
    jpg_quality=90,
    max_frames=None
):
    ...
    # 시작/종료 프레임
    start_f = int(start_sec * src_fps) if start_sec is not None else 0
    end_f   = int(end_sec * src_fps) if end_sec is not None else total - 1
    end_f   = end_f if end_f >= 0 else total - 1
    ...
    while True:
        ...
        if (fidx - start_f) % step == 0:
            ...
            out_path = out_dir / f"frame_{fidx:06d}.jpg"
            ok = imwrite_unicode(out_path, img, [cv2.IMWRITE_JPEG_QUALITY, jpg_quality])
            if ok:
                saved_paths.append(str(out_path))
                keep += 1
                if max_frames and keep >= max_frames:
                    break
...
if __name__=="__main__":
    ...
    output_depth_npy = os.path.join(output_dir, "depth.npy")
    output_depth_vis = os.path.join(output_dir, "depth_vis.jpg")
    output_overlay   = os.path.join(output_dir, "yolo_depth_overlay.jpg")
    output_ply_path = os.path.join(output_dir, "point_cloud.ply")

    video_path = "data/지노드론내서읍.mp4"
    frames_dir = "data/frames"
    paths = extract_frames(
        video_path,
        frames_dir,
        every_n=1,         # 모든 프레임 저장
        target_fps=None,         # 초당 2장만 뽑아서 I/O 부담 줄이려면 2를 넣으면 됨 
        resize=None,          # 필요하면 (1280,720) 등
        jpg_quality=92
    )

    ### 아래 파라미터는 실제 파라미터로 바꿔야 함 !!! ###
    Cw = [100.0, 200.0, 5.0]     # 드론의 월드 좌표 (E, N, U) [m]
    yaw_deg = 30.0                # 방위(0°=East, +CCW)
    pitch_deg = -20.0             # 하향이면 -, 상향이면 +
    roll_deg = 0.0
    # ----------------------------------
    ...
    for p in tqdm(sorted(paths)):
        ...
        yolo_results = yolo.predict(bgr, conf=yolo_conf)
        ...
        rgb_pil, _, focal_px = depth_pro.load_rgb(p)  # focal_px는 픽셀 단위   ---> 아래 코드 사용 !
        ...
        if run_depth_now:
            ...
            with torch.no_grad():
                ...
                # f_any = result.get("focallength_px", None)
                # if f_any is not None:
                #     cached_fpx = float(f_any.detach().item()) if torch.is_tensor(f_any) else float(f_any)
        ...
        # np.save(output_depth_npy, depth_m)   # 미터 단위 저장

        # # YOLO 추론
        # # yolo = YOLO(yolo_weights) --> del
        # # NOTE: ultralytics는 BGR ndarray 입력을 지원. (원하면 RGB로 변환해도 됨)
        # # yolo_res = yolo.predict(source=bgr, conf=yolo_conf, verbose=False)   --> del

        # overlay = bgr.copy()
        # # yolo_res[0] : 첫 번째 이미지의 Results
        # # 여러개 이미지를 넣었다면,
        #         # for img_idx, res in enumerate(yolo_res):  # 이미지/프레임 단위
        #         #     for i in range(len(res.boxes)):       # 객체(박스) 단위
        # names = yolo_res[0].names
        # detections = yolo_res[0].boxes
        ...
        #         # ground_D, info = ground_range_from_dist(obj3d["dist"], cam_height=2.0, obj_height=0.0)
        ...
        #         # 월드좌표 변환
        #         Pw = cam_to_world_point(X, Y, Z, yaw_deg, pitch_deg, roll_deg, Cw, mount="nadir", R_bc=None)
        ...
        # print(f"✔ 저장: {output_depth_npy}")
            ...
            os.remove(p)  # 처리 후 바로 삭제해서 디스크 사용 최소화
